//! Read-only PDF snapshots and page delivery, shared by the CLI and desktop MCP.
...
impl Document {
    pub fn open(root: &Path, path: &Path, expected_sha256: Option<&str>) -> Result<Self, String> {
        ...
        let (info, warnings) = run_poppler(Command::new("pdfinfo").arg(snapshot.path()))?;
    ...
    pub fn read_pages(&self, start: u32, count: u32) -> Result<Vec<PageText>, String> {
        ...
        (start..=end).map(|page| {
            let (output, warnings) = run_poppler(Command::new("pdftotext")
