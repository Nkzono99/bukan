...
@pytest.mark.parametrize("command", ["request", "desktop"])
...
def test_cli_json_stdio_contract_and_errors_without_traceback(store, command, payload, success):
    completed = subprocess.run([sys.executable, "-m", "bukan_research.cli", "--store", str(store.path), command],
...
@pytest.mark.parametrize("option", ["--s", "--st", "--sto", "--stor"])
@pytest.mark.parametrize("attached", [False, True])
def test_cli_rejects_abbreviated_store_override_without_creating_data(tmp_path, option, attached):
    bound = tmp_path / "bound" / "research.sqlite"
    alternate = tmp_path / "alternate" / "research.sqlite"
    override = [f"{option}={alternate}"] if attached else [option, str(alternate)]
    completed = subprocess.run(
        [sys.executable, "-m", "bukan_research.cli", "--store", str(bound), *override, "init"],
        encoding="utf-8", capture_output=True, env={**os.environ, "PYTHONUTF8": "1"}, check=False,
    )
    assert completed.returncode == 2
    assert completed.stderr and "Traceback" not in completed.stderr and not completed.stdout
    assert not bound.parent.exists() and not alternate.parent.exists()


@pytest.mark.parametrize("command, author", [("request", "cli-user"), ("desktop", "desktop-user")])
def test_cli_json_note_edits_preserve_author_and_pinned_provenance(store, command, author):
    original = store.get("n")
    payload = {"version": 1, "operation": "save-note", "id": "n", "expectedRevision": 1,
               "markdown": "Saved through the JSON command."}
    completed = subprocess.run([sys.executable, "-m", "bukan_research.cli", "--store", str(store.path), command],
                               input=json.dumps(payload), encoding="utf-8", capture_output=True,
                               env={**os.environ, "PYTHONUTF8": "1"}, check=False)
    assert completed.returncode == 0, completed.stderr
    assert json.loads(completed.stdout)["version"] == 1
    assert store.get("n")["author"] == author
    assert store.get("n")["entity"] == {**original["entity"], "markdown": payload["markdown"]}
    assert store.get("n", 1) == original


def test_legacy_module_preserves_request_results_and_author(store):
    from bukan_research.desktop import handle_request as legacy_request

    payload = {"version": 1, "operation": "get", "id": "n"}
    assert legacy_request(store, payload) == handle_request(store, payload)
    saved = legacy_request(store, {"operation": "save-note", "id": "n", "expectedRevision": 1,
                                   "markdown": "Saved through the compatibility module."})
    assert saved["version"] == 1 and saved["revision"] == 2
    assert store.get("n")["author"] == "desktop-user"
