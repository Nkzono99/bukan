...
pub fn create_review(
    ...
    let title = title
        ...
        .unwrap_or(&theme)
        .to_string();
    let reviews_root = crate::storage::validate_workspace_destination(
        &workspace_root,
        &reviews_root(&workspace_root),
    )?;
    ...
    validate_review_writes(&workspace_root, &directory, 1)?;
...
pub fn update_review(
    ...
    validate_review_writes(&workspace_root, &directory, metadata.revision)?;
...
pub fn attach_figure(
    ...
    let destination = crate::storage::validate_workspace_destination(
        &workspace_root,
        &directory.join("figures").join(&file),
    )?;
...
pub fn set_current_review(workspace_root: &Path, review_id: &str) -> Result<PathBuf, String> {
    ...
    let path = crate::storage::validate_workspace_destination(
        &workspace_root,
        &workspace_root.join(".bukan/current-review.md"),
    )?;
    let context_directory = path.parent().expect("context file parent");
...
fn validate_review_writes(
    ...
    for relative in [
        ...
        "review.json".into(),
        "figures".into(),
        format!("history/revision-{revision:04}.md"),
    ] {
        crate::storage::validate_workspace_destination(workspace_root, &directory.join(relative))?;
    }
    Ok(())
}
...
fn initial_article(title: &str, theme: &str) -> String {
    format!(
        "# {title}\n\n\
         ...
         ## Evidence synthesis\n\n\
         Bukan MCPから文献を検索し、重要な主張には `[@paper-id, p. 12]` の形式で出典位置を付けて更新します。\n\n\
         ## Open questions\n\n\
         - 未解決の論点\n\n\
         ## References\n\n\
...
mod tests {
    ...
    fn refuses_reviews_and_history_outside_workspace() {
        let temporary = test_workspace();
        let root = temporary.path().join("workspace");
        let outside = temporary.path().join("outside");
        fs::create_dir(&outside).unwrap();
        fs::remove_dir(root.join("reports")).unwrap();
        crate::storage::link_directory(&outside, &root.join("reports"));
        assert!(create_review(&root, "Escaped review", None).is_err());
        assert_eq!(fs::read_dir(&outside).unwrap().count(), 0);

        let safe_root = temporary.path().join("other-workspace");
        workspace::init_workspace(&safe_root, None, Some("offline-source")).unwrap();
        let review = create_review(&safe_root, "Safe review", None).unwrap();
        let directory = Path::new(&review.directory);
        fs::remove_dir(directory.join("history")).unwrap();
        crate::storage::link_directory(&outside, &directory.join("history"));
        assert!(update_review(&safe_root, &review.id, "changed", None).is_err());
        assert_eq!(
            fs::read_to_string(directory.join("article.md")).unwrap(),
            review.article
        );
        assert_eq!(fs::read_dir(&outside).unwrap().count(), 0);
    }
